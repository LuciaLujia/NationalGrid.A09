...
eu_1920_raw <- rbind(eu_2019, eu_2020) %>% 
...
# Convert flow and generation data to daily
flow_1920_day <- as.data.frame(rbind(flow_2019, flow_2020)) %>%
  mutate(Datetime= floor_date(Datetime, "day")) %>% 
  group_by(Datetime) %>% 
  summarise_at(1:59, sum, na.rm=TRUE) 
  
eu_1920_day <- eu_1920_raw %>% 
  mutate(Datetime= floor_date(Datetime, "day")) %>% 
  group_by(Country, Datetime) %>% 
  summarise_at(1:21, sum, na.rm=TRUE) 
