...
ggplot(gb_carbon_savings, aes(x= Interconnector, y= `Carbon Savings`/1e6, fill= year)) +
  ...
  geom_text(aes(label = round(`Carbon Savings`/1e6, 2), vjust = 1.5) ,colour = "black", position = position_dodge(.9), size = 4)+
  theme(
  ...
  scale_fill_manual(values=c("#878787", "#3288bd"), name = "Year")
...
ggplot(gb_zero_carbon, aes(x= Country, y= zero_carbon_share, fill= Year, label = scales::percent(zero_carbon_share))) +
  ...
  theme_clean()+
   labs(
    title="Average Share of Zero Carbon Energy in European Markets Connected to Great Britain",
    subtitle= "2019* vs. 2025",
  ...
  geom_text(aes(vjust = 1.3) ,colour = "black", position = position_dodge(.9), size = 3)+
  theme(
    axis.text=element_text(size=12),
    axis.title=element_text(size=12),
    plot.title = element_text(color="black", size=16, face="bold"),
  ...
  scale_fill_manual(values=c("#878787", "#3288bd"))+
...
ggplot(gb_carbon_intensity_2025, aes(x=Country, y= `Carbon Intensity` , fill= Type, label =round(`Carbon Intensity`,2)))+
  ...
  scale_fill_manual(values=c("#878787", "#3288bd"), name="Forecast Type")
