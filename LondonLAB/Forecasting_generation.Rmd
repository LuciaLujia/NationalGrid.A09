...
# Model building
Model <- VAR(v1, p = 5, type = "const", season = NULL, exog = NULL) 
...
# Create time series for each variable (exclude total generation)
# Biomass.ts <- as.ts(data1.NO["Biomass"]) 
Fossil.Gas.ts <- as.ts(data1.NO["Fossil.Gas"]) 
# Fossil.Hard.coal.ts <- as.ts(data1.NO["Fossil.Hard.coal"])
# Nuclear.ts <- as.ts(data1.NO["Nuclear"]) 
Other.ts <- as.ts(data1.NO["Other"]) 
# Solar.ts <- as.ts(data1.NO["Solar"]) 
...
# Nuclear.ts <- as.ts(data1.DK["Nuclear"]) 
# Other.ts <- as.ts(data1.DK["Other"]) 
Solar.ts <- as.ts(data1.DK["Solar"]) 
# Hydro.ts <- as.ts(data1.DK["Hydro"]) 
...
all_pred <-rbind(result.GB, result.BE, result.NO, result.FR, result.NL, result.DK, result.IE) %>% 
  mutate(weighted_carbon = fcst * carbon_intensity)

all_pred_carbon <- all_pred %>%
  group_by(Country, time) %>% 
  mutate(carbon = sum(weighted_carbon)/sum(fcst)) %>% 
