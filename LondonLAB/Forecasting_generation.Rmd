...
Biomass.ts <- as.ts(data1.GB["Biomass"]) # not seasonal
Fossil.Gas.ts <- as.ts(data1.GB["Fossil.Gas"]) # seasonal 
Fossil.Hard.coal.ts <- as.ts(data1.GB["Fossil.Hard.coal"]) # seasonal 
Nuclear.ts <- as.ts(data1.GB["Nuclear"]) # not seasonal
Other.ts <- as.ts(data1.GB["Other"]) # not seasonal
Solar.ts <- as.ts(data1.GB["Solar"]) # seasonal 
Hydro.ts <- as.ts(data1.GB["Hydro"]) # seasonal 
Wind.ts <- as.ts(data1.GB["Wind"]) # seasonal 
# total_generation.ts <- as.ts(data1.GB["total_generation"]) # seasonal 

v1 <- cbind(Biomass.ts, Fossil.Gas.ts, Fossil.Hard.coal.ts, Nuclear.ts, Other.ts,
            Solar.ts, Hydro.ts, Wind.ts)

colnames(v1) <- cbind("Biomass","Fossil.Gas","Fossil.Hard.coal", "Nuclear",
                      "Other", "Solar", "Hydro", "Wind")
...
fanchart(forecast, names = "Fossil.Gas", main = "Fanchart for Fossil.Gas", xlab = "Horizon", ylab = "Biomass")
autoplot(Fossil.Gas.ts)
